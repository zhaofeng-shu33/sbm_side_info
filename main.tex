...
Now we construct a solution pair to (\ref{eq:sdp}, \ref{eq:sdp_dual}): $X=\tilde{g}\tilde{g}^T, y_{i+1} = \tilde{B}(\tilde{g}\tilde{g}^T)_{ii}$.
Then $$
...
and a semi-definite programing algorithm which, though sub-optimal in general, shows optimality in cases where the Kullback-Leibler (KL)-divergence between $P_0$ and $P_1$ is symmetric, i.e., $D(P_0||P_1)=D(P_1||P_0)$.
