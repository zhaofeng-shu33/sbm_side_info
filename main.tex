...
\begin{document}
	...
	Firstly, we derived a sharp threshold for the balanced SBM with side information. It turns out that the threshold in this paper coincides with that of \cite{abbe17sideinfo} for the special case of SBM with side information and without balanced constraint. 
	Secondly and more importantly, we proposed an SDP based algorithm that achieves the threshold with high probability. Our SDP algorithm is a generalization of the SDP in \cite{abbe2015exact} where side information is included. Yet, different from the SDP algorithms proposed in \cite{esmaeili2019exact,esmaeili2019b,esmaeili2020a}, where the side information consists of a constant number of samples, our SDP algorithm deals with cases where the number of samples grows along with the number of nodes $n$. The latter cases pose challenges different from the former cases, and the algorithms in \cite{esmaeili2019exact,esmaeili2019b,esmaeili2020a} do not work in such cases.
...
%	the contribution of side information to exact recovery error bound in these two cases.
	...
	\end{definition}
	The following definition will be used throughout this paper and is a special case of the definition in \cite{abbe17sideinfo}. Define $I_+$ to be 
...
\end{equation}
	In addition, we need another definition from \cite{abbe2015exact}.
	For a real number $\epsilon$, let 
...
\end{proof}
	...
	% \end{corollary}
 	...
 	The Fig. \ref{fig:my_label} shows that the empirical probability of successful recovery of the SDP algorithm 
 	...
 	the exact recovery metric. In blue, we plot the curve corresponding to the threshold $\sqrt{a}-\sqrt{b}=\sqrt{2}$ while in red we draw the curve corresponding to the threshold $I_+ > 1$. It is clearly seen
 	from the figure that the recovery threshold
 	for SBM with side information is given by the red line $I_+ > 1$.
 	\begin{figure}
 	    \centering
 	    ...
 	    \label{fig:my_label}
 	\end{figure}
\begin{proof}[Proof of Lemma \ref{lem:p0p12}]
