...
&> 3a^{1/3}b^{1/3}(a^{1/6}-b^{1/6})^2 + \gamma D_{1/2}(p_0||p_1)\label{eq:two_conditions}
\end{align}
...
decreasing of error probability $P_e$. Under some parameter configurations specified in \eqref{eq:two_conditions},
...
& \stackrel{(a)}{=} (\sum_{x\in \mathcal{X}} p_0^{1-s}p_1^{s})^{km} (\sum_{x\in \mathcal{X}} p_1^{1-s}p_0^{s})^{km} \\
& \cdot \exp\left(k\log n (1-\frac{2k}{n})(-a-b+a^sb^{1-s}+b^sa^{1-s} +o(1))\right)
\end{align*}
