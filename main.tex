...
\begin{lemma}\label{lem:zxt}
	...
	P(Z - X \geq t \log n) = \exp(-\log n \frac{m}{n}\cdot ( g(a, b, \frac{n}{m}t) + O(\frac{\log n}{n})))
	\end{equation}
...
\begin{align*}
&P(G_1(i,j)) = P(\sum_{r=1}^{n} (z'_r - z_r) \geq  \epsilon \log n) \\
& \geq Q^m_0 (T(P^{(1)})) Q^m_1 (T(P^{(2)})) 
 P(\sum_{r=1}^{n} (z'_r - z_r) \geq \epsilon(P^{(1)}, P^{(2)}) \log n) \\
 & \geq \frac{1}{|\mathcal{P}_{m}|^2} \exp(-m(D(\widetilde{X}_1 || p_0) + D(\widetilde{X}_2 || p_1))
 \exp(- \log n (g(a, b, \epsilon))) \\
 & = \exp(-\log n (\tilde{\theta} + o(1)))
\end{align*}
where we choose $P^{(1)}=P^{(2)}$, specified by \eqref{eq:p012} such that the exponent $\tilde{\theta}$
is reached:
\begin{equation*}
