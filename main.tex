...
\begin{lemma}\label{lem:p0p12}
	...
	of $D_{\KL}(X||p_0) + D_{\KL}(X||p_1)$ for any discrete random variable $X$ is
...
Since $\theta^*>0$,
and $|P_n|\leq (km)^{2|\mathcal{X}|} $, we have $P(A_k) \leq n^{-\theta^*+o(1)}$.
...
& \leq \exp(k(-\mu \log n + \frac{2k}{n} \log n(\sqrt{a} - \sqrt{b})^2 - 2\log 2k + 2))
\end{align*}
\end{proof}
