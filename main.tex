...
is to investigate the gain that extra information brings to the detection problem, compared to the case where only edge observation is available.

	% first paragraph: short intro to SBM 
To get theoretical insight into such a problem, it is often assumed that the graph is generated from a simple probabilistic model called Stochastic Block Model (SBM), in which the probability of edge existence is higher within the community than that between different communities \cite{holland1983stochastic}. For the sole presence of SBM, as the size of community grows, the error rate decreases in both the exact recovery and weak recovery metric. For a special case of two community model,
the weak recovery error rate has been obtained as $n^{-(\sqrt{a} - \sqrt{b})^2/2}$ where $a,b$ are parameters of SBM \cite{zhang2016}.

With the presence of extra node information, the condition of exact recovery is improved
and generalized \cite{saad2018community}. However, previous study does not exactly quantify the error rate of SBM with side information. 
The error rate, achieved by maximum likelihood method, is the fundamental limit of recovery algorithms. It is known that many algorithms like variants of SDP relaxation and
spectral clustering can achieve exact recovery but their error rate may not achieve the fundamental limit. The study of error rate provides a unified way to compare
different algorithms at least in experiment level.
This paper will fill the gap by considering a model of two-community SBM with extra node feature vectors. We have obtained that
...
Rényi divergence has been used in SBM in \cite{zhang2016} to characterize the weak recovery error rate. Both the dense and sparse graph are considered. In this paper, we consider the exact recovery metric and obtain similar results containing
Rényi divergence
...
In contract, ML estimator for $y\in \{ \pm 1 \}^n$ (unrestricted parameter space) in considered in \cite{abbe17sideinfo} if the prior distribution
of each $Y_i \sim Binom(\frac{1}{2})$.
...
counterpart, which uses $\dist(\hat{Y}, Y)/n$ as the error rate.
...
The result for the second case of sparse graph
is summarized
...
In this paper, we obtain a close-form exact recovery error rate for a two-community SBM with side information. This rate
...
\end{proof}
Before proof of theorem \ref{thm:Pe}, we introduce the following lemma, whose proof can follow
in.
...
\end{lemma}
\begin{proof}
	When $k=1$, the left hand side of \eqref{eq:ein} can be expressed as
	\begin{align*}
	\epsilon\triangleq&m\cdot [(D(P_{\widetilde{X}^{i_1}} || P_0) - D(P_{\widetilde{X}^{i_1}} || P_1)) \\
	&+(D(P_{\widetilde{X}^{i_2}} || P_1) - D(P_{\widetilde{X}^{i_2}} || P_0))],
	\end{align*}
	where $P_{\widetilde{X}^i}$ is the empirical distribution of samples $\{x^i_j\}^m_{j=1}$ at node $i$. Let $P_{\widetilde{X}^{i_1}}$ and $P_{\widetilde{X}^{i_2}}$ follow the distribution
