...
%		\newpage
		...
		\begin{align}
			&\sum_{j=1}^{m} \log \frac{P_1(x^{i_1}_{j})}{P_0(x^{i_1}_{j})}
			+\sum_{j=1}^{m} \log \frac{P_0(x^{i_2}_{j})}{P_1(x^{i_2}_{j})}\nonumber\\
			\ge &\log \frac{p(1-q)}{q(1-p)}(2\frac{\log n}{\log\log n}+2+E(i_1, S_1 \backslash H_1) + E(i_2, S_2 \backslash H_2)\nonumber\\
			&- E(i_1, S_2) - E(i_2, S_1)) \label{eq:G1ij}.
		\end{align}
		with high probability.
				Instead of considering the above property directly, we split \eqref{eq:G1ij} into two parts
		\begin{align}
			\sum_{j=1}^{m} \log \frac{P_1(x^{i_1}_{j})}{P_0(x^{i_1}_{j})}
			\geq & \log \frac{p(1-q)}{q(1-p)}(\frac{\log n}{\log\log n} + 1 + \notag\\
			& E(i_1, S_1 \backslash H_1) - E(i_1, S_2)) \label{eq:single_side_Gi}\\
			\sum_{j=1}^{m} \log \frac{P_0(x^{i_2}_{j})}{P_1(x^{i_2}_{j})}
			\geq &  \log \frac{p(1-q)}{q(1-p)}(\frac{\log n}{\log\log n} + 1 + \notag \\
			& E(i_2, S_2 \backslash H_2)  - E(i_2, S_1))\label{eq:single_side_Gai}
		\end{align}
		
		Let $G(i_1,i_2)$ be the event that \eqref{eq:G1ij} holds for any $i_1\in H_1$ and $i_2\in H_2$,
		$G(i_1)$(or $G'(i_2)$) be the event that \eqref{eq:single_side_Gi} (or \eqref{eq:single_side_Gai}) holds for any $i_1 \in H_1$(or $i_2 \in H_2$).
		Since $P(\cup_{i_1\in H_1, i_2\in H_2} G(i_1,i_2)) \le P(\cup_{i_1\in H_1, i_2\in H_2} F(i_1,i_2))$, we only need to show $P(\cup_{i_1\in H_1, i_2\in H_2} G(i_1,i_2))\to 1$, which
		implies $P(\cup_{i_1\in H_1, i_2\in H_2} F(i_1,i_2)) \to 1$.
		Notice that the intersection of the event $\cup_{i_1 \in H_1} G(i_1)$
		and $\cup_{i_2 \in H_2} G'(i_2)$ implies $\cup_{i_1\in H_1, i_2\in H_2} G(i_1,i_2)$.
		The probability of this intersection event is further lower bounded by
		$P(\cup_{i_1 \in H_1} G(i_1)) + P(\cup_{i_2 \in H_2} G'(i_2)) - 1$. Therefore,
		we only need to show $P(\cup_{i_1 \in H_1} G(i_1)) \to 1$ to finish the proof.
		Suppose that
		the probability of $G(i_1)$ is lower bounded by $\frac{1}{n^{-1+\mu}}$ for some positive constant $\mu$, then we have that 
		\begin{align}
			& P(\cap_{i_1 \in H_1} G^c(i_1))= (1 - P(G(i_1,i_2)))^{|H_1|}\nonumber\\
			& \leq (1-n^{-1+u})^{n/\log^3 n} \leq \exp(-n^{\mu+ o(1)}) \to 0, \label{eq:boundg}
		\end{align}
		where  $G^c(i_1)$ is the complement of the event $G(i_1)$. Eq. \eqref{eq:boundg} implies that with high probability $G(i_1)$ occurs for some $i_1 \in H_1$, and hence the failure occurs.
